...
cv::Mat adjustGamma(const cv::Mat& img, double gamma) {
    CV_Assert(gamma > 0); // Verifica se o gamma é válido
    cv::Mat img_gamma;
    cv::Mat lut(1, 256, CV_8U); // Tabela de mapeamento para intensidades

    // Preenche a tabela de mapeamento com a correção de gamma
    for (int i = 0; i < 256; i++) {
        lut.at<uchar>(i) = cv::saturate_cast<uchar>(pow(i / 255.0, gamma) * 255.0);
    }

    // Aplica a tabela de mapeamento à imagem
...
int main(int argc, char** argv)
{
  ...
  //firstImuState.g << 6.3621103117506,-0.810551558752998,7.16546762589928;
  firstImuState.g << 0,0,-9.81;
...
#define TRECORD(X) {float T = (end_##X-start_##X).toSec();\
    ...
    auto image_data = std::get<1>(data_pack);

    if (imu_reading) {
        printf("IMU timestamp\n");
    }
  if(std::get<1>(data_pack)){
        ...
        // Obtenção da imagem (grayscale CV_8U)
        cv::Mat img1 = std::get<1>(data_pack).get();

        // Cria o objeto CLAHE (CPU) uma vez
        double clipLimit    = 8;
        cv::Size tileGridSize(16, 16);
        
        static cv::Ptr<cv::CLAHE> clahe = cv::createCLAHE(clipLimit, tileGridSize);

        // Aplica CLAHE em img1 e guarda em img
        cv::Mat img2;
        clahe->apply(img1, img2);
        cv::Mat img = adjustGamma(img2, 2);

        //cv::GaussianBlur(img, img, cv::Size(5, 5), 1.5);        


        // A partir daqui, 'img' é a tua imagem com contraste aumentado
      ...
      // Cria o objeto CLAHE (CPU) uma vez
      double clipLimit    = 16.0;
      cv::Size tileGridSize(32, 32);
      static cv::Ptr<cv::CLAHE> clahe = cv::createCLAHE(clipLimit, tileGridSize);

      // Aplica CLAHE em img1 e guarda em img
      cv::Mat img;
      clahe->apply(img1, img);
      cv::GaussianBlur(img, img, cv::Size(5, 5), 1.5);
