...
namespace msckf_mono
{
    ...
    prev_imu_time_(0.0)
  {
    ...
    track_info_pub_ = nh.advertise<TrackInfo>("track_info", 10);
  ...
  void RosInterface::imageCallback(const sensor_msgs::ImageConstPtr& msg)
  {
    ...
    publish_extra(msg->header.stamp, cur_features.size(), new_features.size());
  }
  ...
  void RosInterface::publish_extra(const ros::Time& publish_time, uint32_t tracked_features, uint32_t new_features)
  {
    ...
    if (track_info_pub_.getNumSubscribers() > 0) {
      TrackInfo out_info;
      out_info.header.frame_id = "cam0";
      out_info.header.stamp = publish_time;
      //out_info.img.header = out_info.header;
      //out_info.img.encoding = sensor_msgs::image_encodings::TYPE_8UC3;
      //out_info.img.data = track_handler_->get_track_image();
      out_info.tracked_features = tracked_features;
      out_info.new_features = new_features;
      track_info_pub_.publish(out_info);
    }
  }
