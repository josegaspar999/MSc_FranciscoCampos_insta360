...
% Undistort pixel coordinates using the thin prism model (iterative).
% Input:  uvd - 2xN distorted pixel coordinates
% Output: uvu - 2xN undistorted pixel coordinates

% 1. Normalize to fisheye-projected space
ud = (uvd(1,:) - camera.Cx) ./ camera.fx;
vd = (uvd(2,:) - camera.Cy) ./ camera.fy;

% 2. Iterative undistortion (fixed-point, 20 iterations)
u = ud;
v = vd;
