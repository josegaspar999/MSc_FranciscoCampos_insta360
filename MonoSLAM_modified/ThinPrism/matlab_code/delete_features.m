...
function [filter, features_info ] = delete_features( filter, features_info, step )

% First, list all features that should be deleted
deletion_list = [];
for i = 1:length(features_info)
    bad_ratio = ( features_info(i).times_measured < 0.5*features_info(i).times_predicted ) &&...
            ( features_info(i).times_predicted > 5 );

    age = step - features_info(i).init_frame;

    % Never matched since initialization
    never_seen = ( features_info(i).times_measured == 0 ) && ( age > 10 );

    % Once seen but now out of FOV for too long:
    % (age - times_predicted) counts steps the feature was not predicted = steps out of view
    out_of_view = ( features_info(i).times_measured > 0 ) && ( (age - features_info(i).times_predicted) > 30 );
    
    if bad_ratio || never_seen || out_of_view
