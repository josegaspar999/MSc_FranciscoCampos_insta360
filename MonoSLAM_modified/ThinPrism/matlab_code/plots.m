...
% plot image stuff
set(0, 'CurrentFigure', figure_all);
subplot(im_fig);
hold off;
imagesc(im);
colormap gray;
...
axes_handler = get(gcf,'CurrentAxes');
cam_x = x_k_k(1); cam_y = x_k_k(2); cam_z = x_k_k(3);
hw = 4; hh = 1; hd = 4;  % half-widths: x, y(height), z
% Offset centre in the camera's forward direction (XZ plane) so the camera
% sits slightly behind centre rather than exactly in the middle
R_cw = q2r(x_k_k(4:7));
forward_world = R_cw * [0;0;1];  % camera optical axis in world frame
fwd_xz = [forward_world(1); forward_world(3)];
if norm(fwd_xz) > 1e-6; fwd_xz = fwd_xz / norm(fwd_xz); end
offset = 1.5;  % how far ahead of centre the view shifts
cx = cam_x + offset * fwd_xz(1);
cz = cam_z + offset * fwd_xz(2);
axis([cx-hw cx+hw cam_y-hh cam_y+hh cz-hd cz+hd]);
grid on;
view(-360,0);