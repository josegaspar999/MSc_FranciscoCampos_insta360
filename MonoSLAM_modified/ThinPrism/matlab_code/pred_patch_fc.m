...
%     H_Wk = [R_Wk r_Wk;zeros(1,3) 1];
    ...
    % Back-project pixel to 3D ray direction via inverse fisheye
    uf1 = (uv_p_f(1) - cam.Cx) / cam.fx;
    vf1 = (uv_p_f(2) - cam.Cy) / cam.fy;
    r1  = sqrt(uf1^2 + vf1^2);
    if r1 > 1e-12
        n1 = -[uf1*(tan(r1)/r1); vf1*(tan(r1)/r1); 1];
    else
        n1 = -[uf1; vf1; 1];
    end

    uf2 = (uv_p_pred(1) - cam.Cx) / cam.fx;
    vf2 = (uv_p_pred(2) - cam.Cy) / cam.fy;
    r2_ = sqrt(uf2^2 + vf2^2);
    if r2_ > 1e-12
        n2 = -[uf2*(tan(r2_)/r2_); vf2*(tan(r2_)/r2_); 1];
    else
        n2 = -[uf2; vf2; 1];
    end
    n2 = H_kpf_k*[n2;1];
