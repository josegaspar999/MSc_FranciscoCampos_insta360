...
function [ filter, features_info ] = map_management( filter,...
    features_info, cam, im,  min_number_of_features_in_image, step, mask )
...
if measured == 0
    [ filter, features_info ] = initialize_features( step, cam,...
        filter, features_info, min_number_of_features_in_image, im, mask );
 else
    if measured < min_number_of_features_in_image
        [ filter, features_info ] = initialize_features( step, cam,...
            filter, features_info, min_number_of_features_in_image - measured + 5, im, mask );
    end
end