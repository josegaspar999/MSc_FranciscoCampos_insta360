...
function a = dhu_dhrl( camera, Xv_km1_k, yi )
    ...
    phi  = yi(5);
    rho  = yi(6);
    mi   = [cos(phi)*sin(theta_id); -sin(phi); cos(phi)*cos(theta_id)];
    ...
    a = [camera.fx * A / hcz,  camera.fx * B / hcz,  camera.fx * (-A*hcx - B*hcy) / hcz^2;
         camera.fy * B / hcz,  camera.fy * C / hcz,  camera.fy * (-B*hcx - C*hcy) / hcz^2];
