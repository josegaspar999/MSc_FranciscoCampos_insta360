...
uv_c1_und = undistort_fm(uv_c1', cam)';   % Nx2 undistorted pixels

% Inverse fisheye: undistorted pixel -> normalized 3D ray
u = (uv_c1_und(:,1) - cam.Cx) / cam.fx;
v = (uv_c1_und(:,2) - cam.Cy) / cam.fy;
r = sqrt(u.^2 + v.^2);
xn = u; yn = v;
idx = r > 1e-12;
...
uv_c2_und = [cam.fx * u2 + cam.Cx, cam.fy * v2 + cam.Cy];
