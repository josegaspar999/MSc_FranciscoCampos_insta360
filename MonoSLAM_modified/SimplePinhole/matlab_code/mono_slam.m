...
% Output path
outputFolder = 'output';
v = VideoWriter(fullfile(outputFolder, 'results_final147.mp4'), 'MPEG-4');
v.FrameRate = 30;
open(v);
...
for step=initIm+1:lastIm
    ...
    tic; [ filter, features_info ] = map_management( filter, features_info, cam, im, min_number_of_features_in_image, step ); t_map = t_map + toc;
    ...
    % Debug: map size vs visible features
    ...
    tic; features_info = search_IC_matches( filter, features_info, cam, im ); t_match = t_match + toc;
