...
#define TRECORD(X) {float T = (end_##X-start_##X).toSec();\
    ...
    if(imu_reading){
      ...
      if(std::get<1>(data_pack)){
        ...
        std::vector<msckf_mono::Vector3<float>, Eigen::aligned_allocator<msckf_mono::Vector3<float>>> cur_features;
        corner_detector::IdVector cur_ids;
        th.tracked_features(cur_features, cur_ids);

        std::vector<msckf_mono::Vector3<float>, Eigen::aligned_allocator<msckf_mono::Vector3<float>>> new_features;
