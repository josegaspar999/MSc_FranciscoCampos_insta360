...
namespace msckf_mono
{
  ...
  void RosInterface::imageCallback(const sensor_msgs::ImageConstPtr& msg)
  {
    ...
    std::vector<Vector3<float>,
      Eigen::aligned_allocator<Vector3<float>>> cur_features;
    corner_detector::IdVector cur_ids;
    track_handler_->tracked_features(cur_features, cur_ids);

    std::vector<Vector3<float>,
      Eigen::aligned_allocator<Vector3<float>>> new_features;
  ...
  void RosInterface::setup_track_handler()
  {
    ...
    nh_.param<std::string>("mask_path", mask_path, "");
    if (!mask_path.empty()) {
        cv::Mat mask = cv::imread(mask_path, cv::IMREAD_GRAYSCALE);
        if (!mask.empty())
            track_handler_->set_mask(mask);
        else
            ROS_WARN_STREAM("Failed to load mask from: " << mask_path);
    }
