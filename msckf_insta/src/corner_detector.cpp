...
void CornerDetector::detect_features(const cv::Mat& image, std::vector<cv::Point2f>& features)
{
  ...
  for(auto it:nm_corners)
  {
    ...
    if(occupancy_grid_[k])
      continue;
    if(!mask_.empty() && mask_.at<uchar>(xy.y, xy.x) == 0)
      continue;
...
void CornerTracker::track_features(cv::Mat img_1, cv::Mat img_2, Point2fVector& points1, Point2fVector& points2, IdVector& id1, IdVector& id2){
  ...
  for(int i=0; i<status.size(); i++){
    ...
    double dist_threshold = 25.0 * w / 752.0;
    bool in_mask = !mask_.empty() &&
      pt.x >= 0 && pt.x < w && pt.y >= 0 && pt.y < h &&
      mask_.at<uchar>((int)pt.y, (int)pt.x) == 0;
    if(dist>dist_threshold || (status.at(i) == 0)||(pt.x<0)||(pt.y<0)||(pt.x>w)||(pt.y>h) || in_mask)	{
...
void TrackHandler::tracked_features(OutFeatureVector& features, IdVector& feature_ids)
{
  ...
  if(cur_features_.size()>0){
    std::vector<cv::Point3f> undistorted_prev_pts;
    undistortPoints(prev_features_, undistorted_prev_pts);

    std::vector<cv::Point3f> undistorted_cur_pts;
    ...
    std::transform(undistorted_prev_pts.begin(), undistorted_prev_pts.end(), std::back_inserter(prev_features),
        [](const cv::Point3f& pt){return msckf_mono::Vector3<float>{ pt.x, pt.y, pt.z };});
...
void TrackHandler::new_features(OutFeatureVector& features, IdVector& feature_ids) {
  ...
  if(new_features_.size()){
    std::vector<cv::Point3f> undistorted_pts;
...
void TrackHandler::undistortPoints(Point2fVector& in, std::vector<cv::Point3f>& out)
{

  // Note: This is not "undistorting" points anymore. It is taking distorted pixel coordinates and returning unit rays in the camera frame.
  // The function name was kept for consistency with the original code, but it should be noted that it is not actually undistorting points in the traditional sense.

  out.resize(in.size());

  const double fx  = 0.0f;
  const double fy  = 0.0f;
  const double cx  = 0.0f;
  const double cy  = 0.0f;
  const double k1  = 0.0f;
  const double k2  = 0.0f;
  const double p1  = 0.0f;
  const double p2  = 0.0f;
  const double k3  = 0.0f;
  const double k4  = 0.0f;
  const double sx1 = 0.0f;
  const double sy1 = 0.0f;
  ...
  for (size_t i = 0; i < in.size(); ++i) {
      ...
      // 3. Inverse fisheye → unit ray
      double r = std::sqrt(u*u + v*v);

      float X, Y, Z;

      if (r > 1e-12) {
          double sin_t = std::sin(r);
          double scale = sin_t / r;

          X = static_cast<float>(u * scale);
          Y = static_cast<float>(v * scale);
          Z = static_cast<float>(std::cos(r));
      } else {
          X = 0.0f;
          Y = 0.0f;
          Z = 1.0f;
...
void CornerDetector::set_mask(const cv::Mat& mask){
  mask_ = mask;
}

void CornerTracker::set_mask(const cv::Mat& mask){
  mask_ = mask;
}

void TrackHandler::set_mask(const cv::Mat& mask){
  detector_.set_mask(mask);
  tracker_.set_mask(mask);
}
                             ...
                             const OutFeatureVector& new_points_in)
{
  ...
  for (int i=0; i < static_cast<int>(old_points_in.size()); ++i) {
    msckf_mono::Vector2<float> old_point = old_points_in[i].head<2>();
    old_point.array() -= principal_point;
    old_point.array() /= focal_length;
    old_points.block(0, col_iter, 2, 1) = old_point;

    msckf_mono::Vector2<float> new_point = new_points_in[i].head<2>();
...
cv::Mat TrackVisualizer::draw_tracks(cv::Mat image)
{
  ...
  for(auto& track : feature_tracks_)
  {
    ...
    // create consistent bright color for each track using HSV-like mapping
    int hue = (id * 35) % 180;
    ...
    int r = std::max(4, outImage.cols / 120);
    if(search!=predicted_pts_.end()){
      cv::circle(outImage, search->second, r+2, color, r/2+1);
    }
    ...
    for(auto it = track.second.rbegin(); it != track.second.rend(); ++it)
    {
      if(first){
        first = false;
        cv::circle(outImage, *it, r, color, r/2+1);
      }else{
        cv::line(outImage, *it, *prev, color, std::max(1, r/2));
      }
      prev = it;
    }
  }
