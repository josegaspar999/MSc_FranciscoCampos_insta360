...
namespace msckf_mono {
  template <typename _S>
    ...
    public:
      ...
      void update(const std::vector<Vector3<_S>, Eigen::aligned_allocator<Vector3<_S>>> &measurements,
      ...
      void addFeatures(const std::vector<Vector3<_S>, Eigen::aligned_allocator<Vector3<_S>>>& features,
                       const std::vector<size_t>& feature_ids) {
      ...
      void marginalize() {
        if (!feature_tracks_to_residualize_.empty()) {
          ...
          MatrixX<_S> H_o = MatrixX<_S>::Zero(3 * total_nObs - 3 * num_passed,
                                              15 + 6 * cam_states_.size());
          MatrixX<_S> R_o = MatrixX<_S>::Zero(3 * total_nObs - 3 * num_passed,
                                              3 * total_nObs - 3 * num_passed);
          VectorX<_S> r_o(3 * total_nObs - 3 * num_passed);

          Vector3<_S> rep;
          rep << noise_params_.u_var_prime, noise_params_.v_var_prime, noise_params_.u_var_prime;
      ...
      void pruneRedundantStates() {
        ...
        for (auto &feature : feature_tracks_) {
          ...
          jacobian_row_size += 3 * involved_cam_state_ids.size() - 3;
        }
        ...
        for (auto &feature : feature_tracks_) {
          std::vector<size_t> involved_cam_state_ids;
          std::vector<Vector3<_S>, Eigen::aligned_allocator<Vector3<_S>>> involved_observations;
          for (const auto &cam_id : rm_cam_state_ids) {
            auto cam_it = find(feature.cam_state_indices.begin(),
                               feature.cam_state_indices.end(), cam_id);
            if (cam_it != feature.cam_state_indices.end()) {
    ...
    private:
      ...
      void calcMeasJacobian(const Vector3<_S> &p_f_G,
        ...
        MatrixX<_S> H_f_j = MatrixX<_S>::Zero(3 * camStateIndices.size(), 3);
        MatrixX<_S> H_x_j =
          MatrixX<_S>::Zero(3 * camStateIndices.size(), 15 + 6 * cam_states_.size());
        ...
        for (int c_i = 0; c_i < camStateIndices.size(); c_i++) {
          ...
          // New (unit ray):
          _S norm_p = p_f_C.norm();
          Vector3<_S> zhat = p_f_C / norm_p;
          Matrix<_S, 3, 3> J_i = (Matrix3<_S>::Identity() - zhat * zhat.transpose()) / norm_p;
          ...
          Matrix<_S, 3, 3> H_f = -H_x.template block<3, 3>(0, 3);
          H_f_j.template block<3, 3>(3 * c_i, 0) = H_f;

          // Potential indexing problem zone
          H_x_j.template block<3, 6>(3 * c_i, 15 + 6 * (index)) = H_x;
        }

        int jacobian_row_size = 3 * camStateIndices.size();
      ...
      VectorX<_S> calcResidual(const Vector3<_S> &p_f_G,
                               ...
                               const std::vector<Vector3<_S>, Eigen::aligned_allocator<Vector3<_S>>> &observations) {
        // CALCRESIDUAL Calculates the residual for a feature position

        VectorX<_S> r_j = VectorX<_S>::Constant(3 * camStates.size(),
        ...
        for (auto state_i : camStates) {
            ...
            Vector3<_S> zhat_i_j = p_f_C / p_f_C.norm();  // predicted unit ray

            r_j.template segment<3>(3 * iter) = observations[iter] - zhat_i_j;
            iter++;
        }
      ...
      bool checkMotion(const Vector3<_S> first_observation,
        ...
        // first_observation is already a unit ray in camera frame
        Vector3<_S> feature_direction = first_cam_pose.linear() * first_observation;
      ...
      void cost(const Isometry3<_S>& T_c0_ci,
                const Vector3<_S>& x, const Vector3<_S>& z,
                _S& e) const {
        // Compute h vector as Equation (37).
        const _S &alpha = x(0);
        const _S &beta = x(1);
        const _S &rho = x(2);
        ...
        // Predict the feature observation as unit ray in ci frame.
        Vector3<_S> z_hat = h / h.norm();
      ...
      void generateInitialGuess(const Isometry3<_S>& T_c1_c2, const Vector3<_S>& z1,
                                const Vector3<_S>& z2, Vector3<_S>& p) const {
        // Linear triangulation using unit ray observations.
        // z1 is the bearing in frame c1, z2 in frame c2.
        // We solve: lambda1 * z1 = R * lambda2 * z2 + t
        // Rearranged: [z1, -R*z2] * [lambda1; lambda2] = t
        Vector3<_S> m = T_c1_c2.linear() * z2;

        Matrix<_S, 3, 2> A_mat;
        A_mat.col(0) = z1;
        A_mat.col(1) = -m;

        // Solve for [lambda1; lambda2] in least-squares sense
        Vector2<_S> lambdas = A_mat.colPivHouseholderQr().solve(T_c1_c2.translation());

        // Feature position in c1 frame
        p = lambdas(0) * z1;
        return;
      }

      bool initializePosition(const std::vector<camState<_S>> &camStates,
                              const std::vector<Vector3<_S>, Eigen::aligned_allocator<Vector3<_S>>> &measurements,
                              Vector3<_S> &p_f_G) {
        ...
        // Check if the solution is valid. Make sure the triangulated
        // point direction agrees with the observation in every camera frame.
        bool is_valid_solution = true;
        for (int i = 0; i < cam_poses.size(); ++i) {
          Vector3<_S> position =
            cam_poses[i].linear() * final_position + cam_poses[i].translation();
          if (position.dot(measurements[i]) <= 0) {
      ...
      void jacobian(const Isometry3<_S>& T_c0_ci,
        ...
        // Unit ray normalization Jacobian: d(h/||h||)/dh = (I - zhat*zhat^T)/||h||
        _S h_norm = h.norm();
        Vector3<_S> z_hat = h / h_norm;
        Matrix3<_S> dz_dh = (Matrix3<_S>::Identity() - z_hat * z_hat.transpose()) / h_norm;

        // Compute the Jacobian of h w.r.t. [alpha, beta, rho]
        Matrix3<_S> W;
        W.template leftCols<2>() = T_c0_ci.linear().template leftCols<2>();
        W.template rightCols<1>() = T_c0_ci.translation();

        // Chain rule: d(z_hat)/d(alpha,beta,rho) = dz_dh * W
        J = dz_dh * W;
      ...
      Vector3<_S> Triangulate(const Vector3<_S> &obs1,
                              const Vector3<_S> &obs2,
                              const Matrix3<_S> &C_12,
                              const Vector3<_S> &t_21_1){
        // Triangulate feature position given 2 unit ray observations and the
        // transformation between the 2 observation frames.
        // obs1 and obs2 are already unit rays.
        Vector3<_S> v1 = obs1;
        Vector3<_S> v2 = obs2;
