...
typedef std::vector<msckf_mono::Vector3<float>,
                    Eigen::aligned_allocator<msckf_mono::Vector3<float>>>
                      OutFeatureVector;
...
class TrackHandler
{
  ...
  private:
    ...
    void undistortPoints(Point2fVector& in, std::vector<cv::Point3f>& out);
